...
function getLabelClass(status) {
  if (status === 'done') return 'px text-[10px] opacity-60 mb-2'
  if (status === 'active') return 'px text-[10px] opacity-80 mb-2 text-black'
  if (status === 'pending_3' || status === 'pending_4') {
    return 'px text-[10px] mb-2 text-[#352E75]'
  }
  return 'px text-[10px] mb-2'
}

function getTitleClass(status) {
  if (status === 'active') return 'px text-[12px] text-black'
  if (status === 'pending_3' || status === 'pending_4') {
    return 'px text-[12px] text-[#352E75]'
  }
  return 'px text-[12px]'
}
...
export function StepItem({
  ...
  const innerClass = `${innerBaseClass} ${positionClass} py-5 pl-10 pr-10 w-full`
