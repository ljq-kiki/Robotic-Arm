...
void loop_fixed_move() {
  ...
  if (Serial.available() > 0) {
    ...
    if (key == 'F') {
      Serial.read(); 
      Serial.println("\n[CMD] Starting Auto-Pick Sequence (F)");

      // 发送上力指令
      all_uart_send_str("#255PULR!");
      smart_delay_with_stop(200); // 修改点 1: delay -> smart_delay_with_stop

      digitalWrite(RELAY_PIN, LOW);

      // --- 准备阶段 ---
      Serial.println(">>> 0. Moving to INIT POS...");
      for (int i = 0; i < 5; i++) set_servo(i, FIXED_INIT_POS[i], FIXED_MOVE_TIME);
      smart_delay_with_stop(FIXED_MOVE_TIME); // 修改点 2: delay -> smart_delay_with_stop

      // --- 步骤 1: 移动到起点 ---
      Serial.println(">>> 1. Moving to START POS...");
      for (int i = 0; i < 5; i++) set_servo(i, FIXED_START_POS[i], FIXED_MOVE_TIME);
      smart_delay_with_stop(FIXED_MOVE_TIME); // 修改点 3
      
      Serial.println(">>> Pause at START...");
      smart_delay_with_stop(MAGNET_PAUSE_TIME); // 修改点 4

      // --- 步骤 2: 电磁铁吸取 ---
      Serial.println(">>> 2. Magnet ON (Sucking)");
      digitalWrite(RELAY_PIN, HIGH);
      smart_delay_with_stop(500); // 修改点 5

      // --- 步骤 3: 移动到终点 ---
      Serial.println(">>> 3. Moving to END POS...");
      for (int i = 0; i < 5; i++) set_servo(i, FIXED_END_POS[i], FIXED_MOVE_TIME);
      smart_delay_with_stop(FIXED_MOVE_TIME); // 修改点 6
      
      Serial.println(">>> Pause at END...");
      smart_delay_with_stop(MAGNET_PAUSE_TIME); // 修改点 7

      // --- 步骤 4: 电磁铁释放 ---
      Serial.println(">>> 4. Magnet OFF (Released)");
      digitalWrite(RELAY_PIN, LOW);
      smart_delay_with_stop(500); // 修改点 8

      // --- 步骤 5: 恢复复位 ---
      Serial.println(">>> 5. Returning to INIT POS...");
      for (int i = 0; i < 5; i++) set_servo(i, FIXED_INIT_POS[i], FIXED_MOVE_TIME);
      smart_delay_with_stop(FIXED_MOVE_TIME); // 修改点 9
