...
import {
  ...
  startMockRun,
  useHardwareStore,
  // 👈 像 TestTool 一样，直接引入我们写好的控制器
  startAssemblyTeachMode,
  setAssemblyReferenceFrame,
  controlAssemblyMagnet,
  executeAssemblyPath
} from '../../services/useHardwareStore.ts'
import { AssemblyModelPageView } from './AssemblyModelPageView.jsx'
...
export default function AssemblyModelPage({ onGoExecution }) {
  ...
  // ==========================================
  // 🔌 1. 硬件连接与卸力（解决 DISCONNECTED）
  // ==========================================
  useEffect(() => {
    const cleanupHardware = initializeHardwareStore()
    startAssemblyTeachMode() // 👈 调用包含 startTelemetry 的函数，瞬间连通
    ...
    const unsubscribeSignal = subscribeHardwareSignal((signal) => {
      ...
      if (waitingP2HardwareSignalRef.current && signal === HARDWARE_SIGNALS.ASSEMBLY_REACHED_SPECIFIED_POINT && stageRef.current === 'second-block' && waypointCountRef.current < 1) {
        clearTimers(); triggerCollision('waypoint'); return
      }
      if (waitingDirectionHardwareSignalRef.current && signal === HARDWARE_SIGNALS.ASSEMBLY_ESTOP_BEFORE_TARGET && stageRef.current === 'second-block') {
        clearTimers(); setHasTriggeredDirectionCollision(true); triggerCollision('direction'); return
      }
      if (waitingSingularityHardwareSignalRef.current && signal === HARDWARE_SIGNALS.ASSEMBLY_SINGULARITY_REACHED && stageRef.current === 'third-block') {
        clearTimers(); setHasTriggeredSingularityCollision(true); setHasSingularityWarning(true); triggerCollision('singularity')
      }
    })

    return () => { cleanupHardware(); unsubscribeSignal(); clearTimers(); setIsRunningPreview(false) }
  }, [])

  // 同步坐标系和电磁铁状态到底层
  useEffect(() => { setAssemblyReferenceFrame(referenceFrame) }, [referenceFrame])
  useEffect(() => { controlAssemblyMagnet(mode === 'pick') }, [mode])

  // ==========================================
  // 🎮 2. Mock 交互辅助与打点
  // ==========================================
  const clearTimers = () => {
    waitingP2HardwareSignalRef.current = false; waitingDirectionHardwareSignalRef.current = false; waitingSingularityHardwareSignalRef.current = false;
  ...
  const triggerCollision = (type) => {
    setIsRunningPreview(false); setHasCollision(true); setShowCollisionToast(true); setShowWrongAnswerToast(false);
    setCollisionHintType(type); setCollisionHintStep(1); setSelectedCollisionOption(null);
  }

  const handlePointChange = (setter, axis, value) => setter((prev) => ({ ...prev, [axis]: value }))
  const handleWaypointChange = (id, axis, value) => setWaypoints((prev) => prev.map((wp) => wp.id === id ? { ...wp, isManuallyEdited: true, point: { ...wp.point, [axis]: value } } : wp))
  const handleJogMove = async (axis, direction) => {
    const distance = axis === 'rx' ? 5 : 10
    if (sendMockJogMove) await sendMockJogMove({ axis, direction, frame: jogFrame, distance })
  }

  // Record 打点：直接用 captureCurrentPoint 抓硬件数据
  const handleRecordPoint = (setter) => setter(captureCurrentPoint())
  const handleRecordWaypoint = (id) => {
    const pt = captureCurrentPoint()
    setWaypoints((prev) => prev.map((wp) => wp.id === id ? { ...wp, point: pt, frame: jogFrame, isManuallyEdited: false } : wp))
  }
  const handleAddWaypoint = () => {
    if (waypoints.length >= MAX_WAYPOINTS) return
    setWaypoints((prev) => [...prev, { id: nextId, point: EMPTY_POINT, frame: jogFrame, isManuallyEdited: false }]); setNextId((id) => id + 1)
  }
  const handleRemoveWaypoint = (id) => setWaypoints((prev) => prev.filter((wp) => wp.id !== id))

  // ==========================================
  // 🚀 3. CONFIRM & TEST (混合执行流)
  // ==========================================
  const handleConfirmTest = () => {
    if (isRunningPreview || hardware.isRunning) return
    if (!canConfirmNow) return
    ...
    if (hasSingularityWarning) {
      if (resetMockRobotToHome) resetMockRobotToHome()
      setHasSingularityWarning(false); setHasCollision(false); setShowCollisionToast(false); setShowWrongAnswerToast(false); setShowCollisionHintModal(false); setSelectedCollisionOption(null); setIsAutomaticReassemblyReady(true); return
    }

    setShowSuccessModal(false); setShowCollisionToast(false); setShowWrongAnswerToast(false); setShowCollisionHintModal(false); setSelectedCollisionOption(null); setIsRunningPreview(true);

    const isRealHardware = hardware.source === 'hardware' && hardware.connection === 'connected'

    if (isRealHardware) {
      // 🚨 如果是真硬件，直接跑我们写好的 executeAssemblyPath
      executeAssemblyPath(grab, waypoints.map(w => w.point), drop, referenceFrame)
      
      // 让定时器等待真实的物理运行时间
      const realTimeMs = 500 + 2000 + 1500 + 1500 + (waypoints.length * 2000) + 2000 + 1500
      waitingP2HardwareSignalRef.current = stage === 'second-block' && waypoints.length < 1
      waitingDirectionHardwareSignalRef.current = stage === 'second-block'
      waitingSingularityHardwareSignalRef.current = stage === 'third-block' && !hasTriggeredSingularityCollision

      runCompleteTimerRef.current = window.setTimeout(() => {
        clearTimers(); setIsRunningPreview(false); setHasCollision(false); setShowSuccessModal(true)
      }, realTimeMs)
      return // 跳过下方 Mock 逻辑
    }

    // --- 以下是完全保留的同事写的 Mock 逻辑 ---
    if (startMockRun) startMockRun(ASSEMBLY_RUN_MS)

    const shouldTriggerWaypointCollision = stage === 'second-block' && waypoints.length < 1
    const shouldTriggerDirectionCollision = stage === 'second-block'
    const shouldTriggerSingularityCollision = stage === 'third-block' && !hasTriggeredSingularityCollision

    if (shouldTriggerWaypointCollision || shouldTriggerDirectionCollision || shouldTriggerSingularityCollision) {
      collisionSignalTimerRef.current = window.setTimeout(() => {
        collisionSignalTimerRef.current = null
        if (shouldTriggerSingularityCollision) {
          setHasTriggeredSingularityCollision(true); setHasSingularityWarning(true); triggerCollision('singularity'); return
        }
        if (shouldTriggerDirectionCollision) {
          setHasTriggeredDirectionCollision(true); triggerCollision('direction'); return
        }
        triggerCollision('waypoint')
      }, shouldTriggerSingularityCollision ? SINGULARITY_SIGNAL_MS : COLLISION_SIGNAL_MS)
      return
  ...
  const handleSuccessPrimaryAction = () => {
    setShowSuccessModal(false)
    if (stage === 'third-block' && isAutomaticReassemblyReady) { if (typeof onGoExecution === 'function') onGoExecution(); return }
    clearTimers(); setShowSuccessModal(false); if (stage === 'first-block') setStage('second-block'); else if (stage === 'second-block') setStage('third-block'); setMode('pick'); setGrab(EMPTY_POINT); setGrabFrame('Base'); setDrop(EMPTY_POINT); setDropFrame('Base'); setWaypoints([]); setNextId(1); setIsRunningPreview(false); setHasCollision(false); setShowCollisionToast(false); setShowWrongAnswerToast(false); setShowCollisionHintModal(false); setSelectedCollisionOption(null); setCollisionHintType('waypoint'); setCollisionHintStep(1); setShowRelativeHintInfo(false); if (stage === 'first-block') setHasTriggeredDirectionCollision(false); if (stage === 'second-block') { setHasTriggeredSingularityCollision(false); setHasSingularityWarning(false); setIsAutomaticReassemblyReady(false); }
  }
  const handleTryAgainCurrentBlock = () => {
    clearTimers(); setShowSuccessModal(false); setHasCollision(false); setShowCollisionToast(false); setShowWrongAnswerToast(false); setShowCollisionHintModal(false); setSelectedCollisionOption(null)
  }

  return (
    <AssemblyModelPageView
      stage={stage} mode={mode} grab={grab} grabFrame={grabFrame} drop={drop} dropFrame={dropFrame} waypoints={waypoints} frameOptions={REFERENCE_FRAME_OPTIONS} jogFrameOptions={JOG_FRAME_OPTIONS} canConfirm={canConfirmNow} isAssemblyRunning={isRunningPreview || hardware.isRunning} hasCollision={hasCollision} showCollisionToast={showCollisionToast} showWrongAnswerToast={showWrongAnswerToast} showCollisionHintModal={showCollisionHintModal} selectedCollisionOption={selectedCollisionOption} collisionHintType={collisionHintType} collisionHintStep={collisionHintStep} showRelativeHintInfo={showRelativeHintInfo} showSuccessModal={showSuccessModal} successPrimaryLabel={stage === 'third-block' && isAutomaticReassemblyReady ? 'Automatic reassembly' : 'Next block'} jogFrame={jogFrame} hasSingularityWarning={hasSingularityWarning} connectionInfo={connectionInfo}
      
      onToggleMode={() => setMode((prev) => (prev === 'pick' ? 'drop' : 'pick'))}
      onConfirmTest={handleConfirmTest} onNextBlock={handleNextBlock} onSuccessPrimaryAction={handleSuccessPrimaryAction} onTryAgainCurrentBlock={handleTryAgainCurrentBlock}
      onOpenCollisionHint={() => setShowCollisionHintModal(true)}
      onCloseCollisionHint={() => { setShowCollisionHintModal(false); setSelectedCollisionOption(null); if (collisionHintType === 'singularity') return; setShowRelativeHintInfo(true) }}
      onSelectCollisionOption={setSelectedCollisionOption}
      onConfirmCollisionHint={() => {
        if (collisionHintType === 'singularity') { if (selectedCollisionOption !== 'C') { setShowWrongAnswerToast(true); return } setShowCollisionHintModal(false); setSelectedCollisionOption(null); setHasCollision(false); setShowWrongAnswerToast(false); return }
        if (collisionHintType === 'direction' && collisionHintStep === 1) { if (selectedCollisionOption !== 'B') { setShowWrongAnswerToast(true); return } setCollisionHintStep(2); setSelectedCollisionOption(null); setShowWrongAnswerToast(false); return }
        if (collisionHintType === 'direction' && collisionHintStep === 2) { if (selectedCollisionOption !== 'A') { setShowWrongAnswerToast(true); return } setShowCollisionHintModal(false); setHasCollision(false); setSelectedCollisionOption(null); setShowRelativeHintInfo(true); setShowWrongAnswerToast(false); return }
        if (selectedCollisionOption !== 'B') { setShowWrongAnswerToast(true); return } setShowCollisionHintModal(false); setHasCollision(false); setSelectedCollisionOption(null); setShowRelativeHintInfo(true); setShowWrongAnswerToast(false)
      }}
      
      onAddWaypoint={handleAddWaypoint} onRemoveWaypoint={handleRemoveWaypoint}
      onChangeGrabAxis={(axis, value) => handlePointChange(setGrab, axis, value)} onChangeGrabFrame={setReferenceFrame}
      onChangeDropAxis={(axis, value) => handlePointChange(setDrop, axis, value)} onChangeDropFrame={setReferenceFrame}
      onChangeWaypointAxis={handleWaypointChange} onChangeWaypointFrame={(id, val) => setWaypoints((p) => p.map(w => w.id === id ? { ...w, frame: val } : w))}
      onChangeJogFrame={setJogFrame} onJogMove={handleJogMove}
      onRecordGrab={() => handleRecordPoint(setGrab)} onRecordDrop={() => handleRecordPoint(setDrop)} onRecordWaypoint={handleRecordWaypoint} showAddWaypoint={waypoints.length < MAX_WAYPOINTS}
    />
  )
}
